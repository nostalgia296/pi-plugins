...
const INIT_PROMPT = `Please analyze this codebase and create a CLAUDE.md file, which will be given to
 future
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("init", {
		description: "Send the built-in init prompt to the agent",
		handler: async (_args, ctx) => {
			if (!INIT_PROMPT.trim()) {
				ctx.ui.notify("Init prompt is empty", "warning");
				return;
			}

			pi.sendUserMessage(INIT_PROMPT);
			ctx.ui.notify("Init prompt sent", "info");
		},
	});
}
